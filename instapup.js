...
async function userCookies() {
  ...
  if (cookiesArr.length !== 0) {
    ...
    jsonfile.writeFile('./usercookies.json', cookies, { spaces: 2 },
    function(err) { 
     if (err) {
...
async function testAvoidtags() {
  ...
  const avoid = await page.$x("//a[contains(text(),'" + config.avoid_hashtags[i] + "')]/@href");
  if (avoid.length>0) {
...
async function exploreAndLike() {
    ...
    await this.page.goto('https://www.instagram.com/explore/tags/'+ config.hastags[(Math.floor(Math.random() * Math.floor(config.hastags.length)))] + '/');
    ...
    for (let i=0; i<100; i++) {  
      ...
      const heart = await this.page.$('span[aria-label="Like"].glyphsSpriteHeart__outline__24__grey_9');
...
async function instaPup() {
  ...
  console.log('Script ran for ' + Math.floor((endTime - startTime)/1000)+ ' s.');
